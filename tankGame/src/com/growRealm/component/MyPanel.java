...
/**
 * 坦克大战绘图区域
 * 为了让panel 不停的重绘子弹，需要将 MyPanel 重绘方法中的内容放入到单独的线程中
 */
public class MyPanel extends JPanel implements KeyListener, Runnable {
    ...
    // 用于存放弹丸，当子弹击中坦克时，就加入一个Bomb 对象
    Vector<Bomb> bombs = new Vector<>();
    // 定义三种图片，用于显示炸弹/弹丸
    Image img1 = null;
    Image img2 = null;
    Image img3 = null;
    // 定义一个存放 EnemyTankInfo 对象的Vector，用于恢复敌人坦克的坐标和方向
    Vector<EnemyTankInfo> enemyTankInfos = null;
    // 敌人坦克数量
    int enemyTankSize = 3;
    ...
    public MyPanel(String type) {
        ...
        if (!file.exists()) {
            ...
            enemyTankInfos = Recorder.getEnemyTankInfos();
        }
        
        // 初始化我的坦克
        heroTank = new HeroTank(150, 150);
        // 设置坦克速度
        heroTank.setSpeed(5);
        switch (type) {
            case "1":
            ...
            case "2": // 继续上局游戏
                // 初始化敌人坦克
                for (int i = 0; i < enemyTankInfos.size(); i++) {
                    EnemyTankInfo enemyTankInfo = enemyTankInfos.get(i);
                    EnemyTank enemyTank = new EnemyTank(enemyTankInfo.getX(), enemyTankInfo.getY());
                    enemyTank.setDirection(enemyTankInfo.getDirection());
                    // 启动敌人坦克线程
                    new Thread(enemyTank).start();
                    // 给该enemyTank 加入一颗子弹
                    Shot shot = new Shot(enemyTank.getX() + 20, enemyTank.getY() + 60, enemyTank.getDirection());
    ...
    public void paint(Graphics g) {
        ...
        if (heroTank != null && heroTank.isLive()) {
            drawTank(heroTank.getX(), heroTank.getY(), g, heroTank.getDirection(), 1);
        }
        // 画出自己射击的子弹
        // 不为空，且处于发射状态
        if (heroTank.getShot() != null && heroTank.getShot().isLive()) {
            g.draw3DRect(heroTank.getShot().getX(), heroTank.getShot().getY(), 2, 2, false);
        }
        if (heroTank.getShots().size() > 0) {
            for (int i = 0; i < heroTank.getShots().size(); i++) {
                Shot shot = heroTank.getShots().get(i);
                if (shot != null && shot.isLive()) {
                    g.draw3DRect(shot.getX(), shot.getY(), 2, 2, false);
                } else {
                    // 如果shot对象已经无效，就从子弹集合中移除
                    heroTank.getShots().remove(shot);
                }
            }
        }
    ...
    public void hitEnemy() {
        if (heroTank.getShot() != null && heroTank.getShot().isLive()) {
            ...
            for (int i = 0; i < enemyTanks.size(); i++) {
                EnemyTank enemyTank = enemyTanks.get(i);
                // 发射单个子弹
                // hitTank(heroTank.getShot(),enemyTank);
                // 发射多颗子弹
                hitTank(heroTank.getShots(), enemyTank);
            }
        }
    }
